  ...
  A.VIEWS['phan-tich'] = function () {
    ...
    const draw = () => {
      ...
      if (an === 'chieusang') {
        ...
        M.draw(PH, A.db.objs.filter(o => o.type === 'duong' || o.type === 'phovanminh' || o.type === 'hem'), { colorFn: () => '#9aa8a3', onClick: o => A.openObj(o.id) });
        gaps.forEach(g => g.pts.forEach(p => L.circleMarker(p, { radius: 5, color: '#d6453b', fillColor: '#d6453b', fillOpacity: .9, weight: 1 }).bindTooltip('Thiếu chiếu sáng: ' + g.road.name).addTo(PH.extra)));
        M.fit(PH, lights);
      } else if (an === 'ngap') {
        ...
        M.draw(PH, A.db.objs.filter(o => o.type === 'thoatnuoc' || o.type === 'hoga'), { colorFn: o => byObj[o.id] ? (byObj[o.id].length >= 3 ? '#d6453b' : '#e5732b') : '#9fb3d9', onClick: o => A.openObj(o.id) });
        Object.keys(byObj).forEach(k => { const o = A.idx.obj.get(k); if (!o) return; L.circle(U.anchor(o), { radius: 60 + byObj[k].length * 50, color: '#d6453b', weight: 1, fillOpacity: .15 + byObj[k].length * .08, interactive: false }).addTo(PH.extra); });
    ...
    if (an === 'chieusang') {
      const { lights, gaps } = lightGaps();
      legend = '<span><i style="background:#f2b01e"></i>Vùng phủ sáng (bán kính 70 m)</span><span><i style="background:#d6453b"></i>Đoạn tuyến thiếu chiếu sáng</span>';
      side = `<div class="stat-row"><div><div class="v">${lights.length}</div><div class="l">đèn hoạt động</div></div><div><div class="v" style="color:#d6453b">${gaps.length}</div><div class="l">tuyến có đoạn thiếu sáng</div></div><div><div class="v">${U.fmtLen(U.sum(gaps, g => g.len))}</div><div class="l">tổng chiều dài thiếu</div></div></div><div class="divider"></div>
      ...
      legend = '<span><i style="background:#d6453b"></i>≥ 3 lần sự cố</span><span><i style="background:#e5732b"></i>1–2 lần</span><span><i style="background:#9fb3d9"></i>Chưa ghi nhận</span>';
      side = `<div class="stat-row"><div><div class="v">${rows.length}</div><div class="l">điểm có sự cố thoát nước</div></div><div><div class="v" style="color:#d6453b">${rows.filter(r => r.n >= 3).length}</div><div class="l">điểm thường xuyên (≥ 3)</div></div></div><div class="divider"></div>
      ...
      legend = '<span><i style="background:#8a6d3b"></i>Bán kính phục vụ 250 m</span><span><i style="background:#7b4bc4"></i>Khu vực</span>';
      ...
      side = `<div class="stat-row"><div><div class="v">${incs.length}</div><div class="l">sự cố đang mở</div></div><div><div class="v" style="color:#d6453b">${incs.filter(i => i.deadline < U.today()).length}</div><div class="l">quá hạn</div></div></div><div class="divider"></div><div class="obj-list">${incs.sort((a, b) => a.deadline.localeCompare(b.deadline)).map(i => `<div class="obj-item" data-act="open-inc" data-id="${i.id}"><div class="t">${U.esc(i.title)}</div><div class="small muted">${i.id} · hạn ${U.dmy(i.deadline)} · ${i.assignee ? U.staffName(i.assignee) : 'chưa phân công'}</div></div>`).join('')}</div>`;
    } else {
