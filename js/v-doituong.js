...
(function (A) {
  ...
  A.VIEWS['nhap-lieu'] = function () {
    ...
    if (imp.step === 0) {
      ...
      body = `<div class="stat-row" style="margin-bottom:12px"><div><div class="v">${imp.rows.length}</div><div class="l">Dòng trong tệp ${U.esc(imp.file)}</div></div><div><div class="v" style="color:#1d7a4f">${ok.length}</div><div class="l">Hợp lệ</div></div><div><div class="v" style="color:#d6453b">${bad.length}</div><div class="l">Có lỗi (bỏ qua khi ghi nhận)</div></div></div>
        ${U.table([{ t: '#' }, { t: 'Mã' }, { t: 'Tên' }, { t: 'Lớp' }, { t: 'Vĩ độ' }, { t: 'Kinh độ' }, { t: 'Hiện trạng' }, { t: 'Kết quả kiểm tra' }],
          imp.rows.map(x => `<tr class="${x.errs.length ? 'row-err' : 'row-ok'}"><td>${x.n}</td><td class="small">${U.esc(x.r.ma || '(tự cấp)')}</td><td>${U.esc(x.r.ten)}</td><td class="small">${x.t ? x.t.name : '<span style="color:#d6453b">' + U.esc(x.r.lop) + '</span>'}</td><td class="small">${U.esc(x.r.vido)}</td><td class="small">${U.esc(x.r.kinhdo)}</td><td class="small">${D.COND[x.cond].label}</td><td>${x.errs.length ? x.errs.map(e => `<span class="tag danger">${e}</span>`).join(' ') : '<span class="tag ok">✓ Hợp lệ</span>'}</td></tr>`))}
